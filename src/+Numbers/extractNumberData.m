...
% Copyright 2018, The MathWorks, Inc.
    ...
    %% Load data
    [images, labels, ~, ~] = src.Semantics.accessSudokuData(1, false);
    pximds = pixelLabelImageDatastore(images, labels);
    dataFile = fullfile(sudokuRoot(), 'data', 'labels', 'numbers.txt');
    digitLabels = src.Numbers.readNumberLabels(dataFile);
    ...
    for iImage = 1:numel(pximds.Images)
        ...
        intersections = Numbers.intersectionsFromLabel(data.pixelLabelImage{1} == "sudoku");
        ...
        % Save images
        [puzzle, repeat] = src.Numbers.parseFilename(filename);
